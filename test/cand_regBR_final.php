...
<body>
    ...
    <h1 style="padding-left: 20%;padding-bottom: 2%;font-size: 50px;color:#1e4f81;font-family: 'Nunito Sans', sans-serif;">BR Candidates Registration</h1>      
...
elseif(isset($_POST['update'])){
  ...
  $Image=$_POST['upload_img'];
    ...
    if ( empty( $Name)OR empty( $Roll_no)OR empty( $Email_id)OR empty( $Batch)OR empty( $dept)OR empty( $Manifesto)OR empty( $Image)) {
     array_push($errors,"*All fields are required");
    }
    if (!filter_var($Email_id, FILTER_VALIDATE_EMAIL)) {
     array_push($errors, "*Email is not valid");
